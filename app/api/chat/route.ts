...
function createModel(providerId: ProviderId, modelId: string) {
  switch (providerId) {
    ...
    case "openai": {
      ...
      // OpenAI-compatible gateways are generally more stable with chat mode.
...
export async function POST(request: Request) {
  try {
    ...
    const analysisMode = payload.analysisMode ?? "claude-skill";
    const resolvedProviderId = resolveProviderId(payload.providerId);
    ...
    const messageInput = Array.isArray(payload.messages)
      ? extractLatestUserText(payload.messages as UIMessage[])
      : "";
    const userInput = (payload.userInput?.trim() || messageInput).slice(0, MAX_USER_INPUT_CHARS);

    if (!userInput) {
      return Response.json(
        {
          error: "提交内容为空，请先填写回答后再提交。",
        },
        { status: 400 },
      );
    }
    ...
    const result = await generateText({
      ...
      system: [
        ...
        payload.sourceFileName
          ? `The source question file is ${payload.sourceFileName}.`
          : "",
        payload.questionText
          ? `The selected source question is: ${payload.questionText}`
          : "",
    ...
    const details = (() => {
      ...
      return {
        ...
        responseBody: maybeError.responseBody,
        cause: maybeError.cause,
      };
    })();

    const parsedGatewayBody = (() => {
      if (!details?.responseBody || typeof details.responseBody !== "string") {
        return null;
      }

      try {
        return JSON.parse(details.responseBody) as {
    ...
    if (parsedGatewayBody?.msg === "Model not support") {
      return Response.json(
        {
          error: [
            "当前网关不支持所选模型。",
            `模型: ${process.env.OPENAI_MODEL ?? "(未配置)"}`,
            `网关: ${process.env.OPENAI_BASE_URL ?? "(未配置)"}`,
            "请在 .env.local 中修改 OPENAI_MODEL 为该网关支持的模型后重试。",
          ].join(" "),
        },
        { status: 400 },
      );
    ...
    return Response.json(
      {
        error: error instanceof Error ? `${error.message}${detailText}` : "对话请求失败。",
      },
      { status: 500 },
    );
  }
